...
class RealisticGroceryListGenerator:
    ...
    def clean_and_parse_ingredient(self, ingredient_text: str) -> Optional[Dict]:
        ...
        # Comprehensive pattern to extract quantity, unit, and ingredient
        pattern = r'^(\d+(?:\.\d+)?(?:\s*-\s*\d+(?:\.\d+)?)?|\d+\s*\d+/\d+|\d+/\d+)?\s*(cups?|cup|tablespoons?|tbsp|teaspoons?|tsp|pounds?|lbs?|lb|ounces?|oz|grams?|g|kilograms?|kg|pieces?|piece|cloves?|clove|heads?|head|bunches?|bunch|cans?|can|jars?|jar|packages?|package|containers?|container|bottles?|bottle|bags?|bag|boxes?|box|slices?|slice)?\s*(.+)
                    ...
                    if line.startswith(('•', '-', '*')) or re.match(r'^\d+\.', line):
                        ...
                        if ingredient_text and len(ingredient_text) > 2:
                            ...
                            if (re.search(r'\b(cup|tbsp|tsp|lb|oz|gram|piece|clove|can|jar|pound|ounce)\b', ingredient_text.lower()) or
                                any(food_word in ingredient_text.lower() for food_word in ['chicken', 'beef', 'pork', 'fish', 'onion', 'tomato', 'potato', 'cheese', 'milk', 'egg', 'rice', 'pasta', 'bread', 'oil', 'salt', 'pepper'])):
                                recipe_data['ingredients'].append(ingredient_text)
                                ingredient_found = True
            ...
            if not ingredient_found:
                for line in lines:
                    ...
                    if (line and len(line) > 5 and
                        not line.lower().startswith(('recipe', 'day', 'breakfast', 'lunch', 'dinner', 'snack', 'ingredients', 'instructions', 'preparation', 'cooking', 'serves', 'servings', 'calories', 'protein', 'carbs', 'fat')) and
                        not line.endswith(':') and
                        # Look for quantity indicators
                        (re.search(r'\d+\s*(cup|tbsp|tsp|lb|oz|gram|piece|clove|can|jar|pound|ounce)', line.lower()) or
                         re.search(r'\b(chicken|beef|pork|fish|salmon|shrimp|onion|garlic|tomato|potato|cheese|milk|egg|rice|pasta|bread|oil|salt|pepper|spinach|broccoli|carrot|bell pepper)\b', line.lower())) and
                        len(line.split()) >= 2 and len(line.split()) <= 8):  # Reasonable ingredient length
                        recipe_data['ingredients'].append(line)
                        ingredient_found = True
            
            # Only add recipe if we found ingredients and a title
