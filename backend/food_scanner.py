...
def get_product_info(barcode):
    """Get product information by barcode with enhanced reliability and fallbacks"""
    try:
        # Validate barcode
        ...
        # Try to fetch from OpenFoodFacts with multiple endpoints and retry logic
        data = analyzer.fetch_from_openfoodfacts(barcode)
        
        if not data:
            # API completely failed - return minimal product with clear error message
            logger.warning(f"All API endpoints failed for barcode: {barcode}")
            minimal_product = analyzer.create_minimal_product_from_barcode(barcode)
            return jsonify(minimal_product), 200  # Return 200 so app doesn't crash
        ...
        minimal_product = analyzer.create_minimal_product_from_barcode(barcode)
        minimal_product['error_details'] = str(e)
        return jsonify(minimal_product), 200  # Return 200 so app doesn't crash
...
def search_products(query):
    ...
    try:
        ...
        }), 200  # Return 200 so app doesn't crash
    except Exception as e:
...
def health_check():
    """Health check endpoint for food scanner with API status"""
    try:
        # Quick test of main API endpoint
        test_response = analyzer.session.get(
    ...
    return jsonify({
        'status': 'healthy',
        'message': 'Enhanced Food Scanner API with Reliability Improvements',
        'version': '4.0',
        ...
        'reliability_improvements': [
            'Multiple OpenFoodFacts server endpoints (world, fr, de, net)',
            'Session-based requests with connection pooling',
            'Exponential backoff retry strategy',
            'Reduced individual request timeouts',
            'Graceful fallback to minimal product data',
            'Always returns HTTP 200 to prevent app crashes'
        ]
    })

# Debug endpoint to test connectivity
@food_scanner_bp.route('/debug-connectivity', methods=['GET'])
def debug_connectivity():
    """Debug endpoint to test all API endpoints"""
    results = {}
    
    for i, endpoint_template in enumerate(analyzer.api_endpoints):
        endpoint_name = f"endpoint_{i+1}"
        url = endpoint_template.format("737628064502")  # Test barcode
        
        try:
            start_time = time.time()
            response = analyzer.session.get(url, timeout=5)
            end_time = time.time()
            ...
            results[endpoint_name] = {
                ...
                'data_available': bool(response.json().get('status') == 1) if response.status_code == 200 else False
            }
        except Exception as e:
            results[endpoint_name] = {
                'url': url,
    ...
    return jsonify({
        ...
        'summary': {
            'total_endpoints': len(analyzer.api_endpoints),
            'successful_endpoints': sum(1 for r in results.values() if r.get('success')),
            'fastest_response_ms': min((r.get('response_time_ms', float('inf')) for r in results.values() if r.get('success')), default=None)
        }
    })
...
def debug_serving_size(barcode):
    ...
    try:
        # Try to fetch from primary endpoint only for debugging
        url = f"https://world.openfoodfacts.org/api/v0/product/{barcode}.json"
        headers = {
            'User-Agent': 'PlateMate-FoodScanner/1.0 (platemate-app@example.com)'
        }
        
        response = analyzer.session.get(url, headers=headers, timeout=8)
        response.raise_for_status()
...
class FoodHealthAnalyzer:
    def __init__(self):
        ...
        self.api_endpoints = [
            ...
            "https://world.openfoodfacts.net/api/v0/product/{}.json",  # Alternative domain
            "https://fr.openfoodfacts.org/api/v0/product/{}.json",     # French server
            "https://de.openfoodfacts.org/api/v0/product/{}.json",     # German server
        ]
    ...
    def fetch_from_openfoodfacts(self, barcode: str) -> Optional[Dict]:
        """
        Fetch product data from OpenFoodFacts with multiple fallback endpoints
        """
        headers = {
            'User-Agent': 'PlateMate-FoodScanner/1.0 (platemate-app@example.com)',
            'Accept': 'application/json',
        ...
        for i, endpoint_template in enumerate(self.api_endpoints):
            url = endpoint_template.format(barcode)
            
            try:
                logger.info(f"Attempting API call {i+1}/{len(self.api_endpoints)}: {url}")
                
                # Reduce timeout for faster failover
                timeout = 8 if i == 0 else 6  # First try gets more time
                
                response = self.session.get(
                    url,
                    headers=headers,
                    timeout=timeout,
                    stream=False  # Don't stream for faster response
                )
                ...
                if response.status_code == 200:
                    ...
                    if data.get('status') == 1 and 'product' in data:
                        logger.info(f"Successfully fetched data from endpoint {i+1}")
                        return data
                    elif data.get('status') == 0:
                        logger.warning(f"Product not found in database (endpoint {i+1})")
                        continue  # Try next endpoint
                    else:
                        logger.warning(f"Invalid response structure from endpoint {i+1}")
                        continue
                        
                else:
                    logger.warning(f"HTTP {response.status_code} from endpoint {i+1}")
                    continue
                    
            except requests.exceptions.ReadTimeout:
                logger.warning(f"Read timeout for endpoint {i+1}")
                continue
            except requests.exceptions.ConnectTimeout:
                logger.warning(f"Connection timeout for endpoint {i+1}")
                continue
            except requests.exceptions.ConnectionError as e:
                logger.warning(f"Connection error for endpoint {i+1}: {e}")
                continue
            except requests.exceptions.RequestException as e:
                logger.warning(f"Request error for endpoint {i+1}: {e}")
                continue
            except Exception as e:
                logger.error(f"Unexpected error for endpoint {i+1}: {e}")
                continue
        
        logger.error(f"All endpoints failed for barcode: {barcode}")
        return None

    def create_minimal_product_from_barcode(self, barcode: str) -> Dict:
        """
        Create a minimal product entry when API fails
        """
        return {
            'barcode': barcode,
            ...
            'serving_info': {
                ...
                'original_value': 'api_unavailable',
                'unit': 'g'
            },
            ...
            'ingredients_analysis': {
                'additives': [],
                'quality_score': 50,
                'warnings': ['Product data unavailable'],
                'ingredient_count': 0
            },
            'nutriments': {},
            'health_score': 50,
            'recommendations': [
                'Product information unavailable - scan barcode again or try manual entry',
                'Check product packaging for nutritional information',
                'Consider choosing products with complete nutritional data'
            ],
            'quality_indicators': {
    ...
    # [Keep all your existing methods: parse_serving_string, get_serving_size_from_api,
    # evaluate_nutrient_level, calculate_health_score, analyze_nutrient_quality,
    # get_level_description, analyze_ingredients, calculate_nutri_score,
    # get_health_recommendations - they remain the same]
